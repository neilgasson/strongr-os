...
export function createStrongrDailyV2FixtureOutput(
	...
	const base = {
		...
		personal_takeaway_prompt:
			"What is one faithful next step you can take today?",
		...
		reflective_transition:
			"Take a slow breath and hold this Scripture in quiet attention.",
...
function createFixtureOutput(request: GenerationRequest) {
	...
	return parseAudioReflection({
		...
		reflection_questions: brief.objectives
			.slice(0, 3)
			.map(
				(objective) => `What would it look like to reflect on: ${objective}?`,
			),
...
export const deterministicGenerationAdapter: GenerationAdapter = Object.freeze({
	...
	generate(request: GenerationRequest): Promise<GenerationResult> {
		...
		const promptChecksum = createGenerationPromptChecksum(
			request.promptKey,
