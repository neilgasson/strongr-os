...
const responseSchema = Object.freeze({
	...
	properties: {
		...
		schema_id: {
			enum: [strongrDailyAudioReflectionV2SchemaId],
			type: "string",
		},
		...
		tone: {
			enum: ["challenging", "encouraging", "pastoral", "reflective"],
			type: "string",
		},
...
function safeFailureCode(status: number): string {
	if (status === 401 || status === 403) {
		return "generation.provider_authentication_failed";
	}
	if (status === 429) {
		return "generation.provider_rate_limited";
	}
	if (status >= 500 && status <= 599) {
		return "generation.provider_unavailable";
	}
...
export function createOpenAiStrongrDailyV2Adapter(
	...
	return Object.freeze({
		...
		async generate(request: GenerationRequest): Promise<GenerationResult> {
			...
			if (!response.ok) {
				throw new GenerationProviderError(safeFailureCode(response.status));
			}
			...
			let output;
			try {
				const candidate = JSON.parse(requireOutputText(body)) as Record<
					string,
					unknown
				>;
				...
				output = parseStrongrDailyAudioReflectionV2({
					...candidate,
			...
			const result: GenerationResult = {
				...
				promptChecksum: createGenerationPromptChecksum(
					request.promptKey,
