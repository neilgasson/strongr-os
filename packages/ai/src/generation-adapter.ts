	...
	if (typeof value === "object") {
		const entries = Object.entries(value as Record<string, unknown>).sort(
			([left], [right]) => {
				...
				return (
					lengthDifference || Buffer.from(left).compare(Buffer.from(right))
				);
			},
		);
		return `{${entries
...
export function createGenerationPromptChecksum(
	promptKey: string,
	promptVersion: number,
): string {
	return createHash("sha256")
		.update(`${promptKey}:${promptVersion}`, "utf8")
		.digest("hex");
}
...
export function createGenerationOutputHash(output: GenerationOutput): string {
	...
	if (output.schema_id === "strongr.strongr_daily_audio_reflection.v2") {
		...
		return createHash("sha256")
			.update(postgresJsonbText(content), "utf8")
			.digest("hex");
	}
	return createHash("sha256")
		.update(postgresJsonbText(output), "utf8")
		.digest("hex");
}
