...
select ok(
  pg_get_functiondef('public.m1_prepare_phase4b5_guided_audio_reflection_brief(uuid,uuid)'::regprocedure)
    like all (array[
      '%m1_phase4b5_require_owner%',
      '%owner_approved_inactive%', '%pg_advisory_xact_lock%',
      '%strongr_daily_phase4b5_temporary_exception_lifecycle%',
      '%expires_at > statement_timestamp()%',
      '%public.m1_create_audio_brief(%',
  ...
  'the preparation command delegates the owner and AAL2 gate and rechecks inactive-profile provenance, the temporary expiry, one-time state, and the exact request hash'
);
select ok(
  pg_get_functiondef('app_private.m1_phase4b5_require_owner(uuid)'::regprocedure)
    like all (array[
      '%require_permission%', '%content.create%', '%true%',
      '%role_record.key = ''owner''%', '%membership_role_revocations%'
    ]),
  'the owner gate rechecks content.create, AAL2, and an unrevoked owner role'
);
select ok(
  pg_get_functiondef('public.m1_prepare_phase4b5_guided_audio_reflection_brief(uuid,uuid)'::regprocedure)
    not like any (array[
      '%insert into public.content_items%', '%insert into public.content_briefs%'
    ]),
  'the temporary wrapper does not create a parallel content write path'
);
select ok(
  pg_get_functiondef('public.m1_prepare_phase4b5_guided_audio_reflection_brief(uuid,uuid)'::regprocedure)
    like '%from public.m1_create_audio_brief(%',
  'the temporary wrapper delegates ordinary brief creation and its audit to the normal governed command'
);
select ok(
  pg_get_functiondef('public.m1_create_audio_brief(uuid,text,jsonb,uuid)'::regprocedure)
    like all (array['%require_permission(p_organization_id, ''content.create'')%', '%content.brief_created%']),
  'the normal Studio brief command and its standard authorization and audit path remain unchanged'
);
select ok(
  (
    select relrowsecurity from pg_class as c join pg_namespace as n on n.oid = c.relnamespace
    where n.nspname = 'app_private' and c.relname = 'strongr_daily_phase4b5_temporary_exception_lifecycle'
  )
  and not has_table_privilege('authenticated', 'app_private.strongr_daily_phase4b5_temporary_exception_lifecycle', 'SELECT,INSERT,UPDATE,DELETE'),
  'the sunset lifecycle is private and cannot be extended by a browser client'
);
select ok(
  pg_get_functiondef('app_private.m1_phase4b5_require_post_test_disposition()'::regprocedure)
    like all (array[
      '%awaiting_removal_or_replacement%',
      '%content.phase4b5_temporary_exception_post_test_disposition_required%',
      '%remove_or_formally_replace%'
    ]),
  'an attempted one-call authorization records the mandatory remove-or-replace disposition'
);
select ok(
