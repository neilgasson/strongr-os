-- Strongr OS
-- Migration: M1.2 brief-to-draft completion
--
-- Atomically binds a validated generation result to one immutable,
-- tenant-scoped AI-assisted draft. Human submission remains a separate
-- authenticated command. Review, approval, export, publication, deployment,
-- and the current Strongr Daily application remain unchanged.

begin;

create unique index content_versions_one_ai_version_per_job_idx
on public.content_versions (organization_id, source_job_id)
...
drop function public.m1_complete_generation_attempt(
  uuid, text, uuid, uuid, text, text, text, integer
);

create function public.m1_complete_generation_attempt(
  p_event_id uuid,
  p_worker_id text,
  p_lease_token uuid,
  p_attempt_id uuid,
...
revoke all on function public.m1_complete_generation_attempt(
  uuid, text, uuid, uuid, text, text, jsonb, text, integer
...
begin
  if to_regprocedure(
    ...
    raise exception
      'M1.2 verification failed: hash-only completion command still exists';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception
      'M1.2 verification failed: draft completion command is missing';
  end if;
  ...
  loop
    if has_function_privilege(
      ...
      raise exception
        'M1.2 verification failed: % can complete generation', v_role;
    end if;
  end loop;
  ...
  if not has_function_privilege(
    ...
    raise exception
      'M1.2 verification failed: service_role cannot complete generation';
  end if;

  if has_table_privilege('service_role', 'public.content_versions', 'INSERT')
     or has_table_privilege('service_role', 'public.content_versions', 'UPDATE')
     or has_table_privilege('service_role', 'public.content_versions', 'DELETE') then
    raise exception
      'M1.2 verification failed: service_role can bypass draft command';
  end if;
  ...
  if not exists (
    ...
    where i.schemaname = 'public'
      and i.tablename = 'content_versions'
      and i.indexname = 'content_versions_one_ai_version_per_job_idx'
  ) then
    raise exception
      'M1.2 verification failed: AI draft idempotency index is missing';
  end if;
end;
$$;
