...
3. `m1_begin_generation_attempt` validates that exact lease before locking the
   generation job and recording attempt provenance.
4. The provider-neutral adapter runs outside the database transaction.
5. `m1_complete_generation_attempt` stores the exact provider response,
   response schema, latency, and output hash for the current attempt.
...
- `m1_fail_generation_attempt` closes the current attempt and moves the job to
  `failed` or `dead_letter`.
- `m0_fail_outbox_event` applies the same retry delay and maximum-attempt
  boundary to delivery.
- An expired running attempt is closed with `worker_lease_expired` before a new
  attempt is created.
...
- The worker receives no direct DML grant. Browser RLS, grants, human
  permissions, AAL2 gates, and approval boundaries are unchanged.
...
- pgTAP tests cover role grants, filtered claims, lease recovery, stale-token
  denial, attempt provenance, idempotent completion, completion-before-ack
  recovery, synchronized retry/dead-letter state, and structured audit events.
...
public command signatures and removes their bodies only after the worker is
stopped. Do not edit an applied migration and do not roll back unrelated M0.2
outbox reliability primitives.
