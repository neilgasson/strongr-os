...
| AAL | SQL JWT simulation and remote TOTP runner | Same privileged command fails at AAL1, without current assurance, and through the stale AAL1 token after MFA; it succeeds at AAL2 and records AAL2 in approval evidence |
| Governed path | `100_m0_2_tenant_aal_governed_e2e.sql` and remote runner | Brief through immutable production package succeeds with exact content, evidence-bundle, and manifest hashes; remote mutation attempts fail |
...
proves inactive-membership and revoked-role denial, executes the governed
path, validates exact assurance/hash evidence and immutability, races
idempotency and outbox workers, exercises retry/crash/duplicate/dead-letter
handling, verifies health and metrics, then removes the fixtures.
...
- `STRONGR_OS_PROJECT_REF` (the 20-character `strongr-os-dev` reference)
- `STRONGR_OS_REMOTE_ACCEPTANCE=strongr-os-dev`
