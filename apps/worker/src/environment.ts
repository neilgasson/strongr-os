...
function resolveGenerationProvider(
	...
	WorkerEnvironment,
	"generationProvider" | "openAiApiKey" | "openAiModel"
> {
	const provider = (
		source.STRONGR_OS_GENERATION_PROVIDER ?? "deterministic"
	).trim();
	...
	const model = requireOpenAiModel(
		requireValue(source, "STRONGR_OS_OPENAI_MODEL"),
	);
	if (apiKey.length < 20) {
...
function rejectPublicPrivilegedNames(source: WorkerEnvironmentSource): void {
	const exposed = Object.keys(source).filter(
		(name) =>
			...
			/(?:SECRET|SERVICE_ROLE|DATABASE_URL|OPENAI)/.test(name),
	);
	if (exposed.length > 0) {
		throw new Error(
			"Privileged worker values cannot use public environment names",
		);
	}
}
...
export function loadWorkerEnvironment(
	...
	return Object.freeze({
		...
		supabaseUrl: requireSupabaseUrl(
			requireValue(source, "STRONGR_OS_SUPABASE_URL"),
		),
