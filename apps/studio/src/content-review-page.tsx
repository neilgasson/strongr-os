...
const ownerReviewSeeds: readonly Item[] = seeds.map((item) => {
  if (!item.id.startsWith("renewal-")) return item;
  const completeSpokenScript = `${item.script}\n\nLet us pray.\n\n${item.prayer}\n\nAs you continue today, take this Daily Practice: ${item.practice}`;
  return {
    ...item,
    script: completeSpokenScript,
    status: "owner_review",
    narrationRights: false,
    versions: [
      `Version ${item.versions.length + 2} — complete spoken source prepared for owner review`,
      `Version ${item.versions.length + 1} — prior private audio draft superseded and not accepted`,
      ...item.versions,
    ],
  };
});
